...
import {ICountryOverview,ICountry} from '../../sharedInterfaces/1-taks-interface'
...
export const countriesApi = async (req: NextApiRequest, res: NextApiResponse<IResult<ICountryOverview[]>>) => {
  ...
  const countries = filterCountries(getAllCountries() as ICountry[],body?.searchTerm?.toLowerCase());
